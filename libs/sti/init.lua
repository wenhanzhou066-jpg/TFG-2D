...
local function parseTSX(data)
	local tileset = {}

	tileset.name = data:match('name="([^"]+)"')
	tileset.tilewidth = tonumber(data:match('tilewidth="(%d+)"'))
	tileset.tileheight = tonumber(data:match('tileheight="(%d+)"'))
	...
	if image_width then
		tileset.imagewidth = tonumber(image_width)
	end
	if image_height then
		tileset.imageheight = tonumber(image_height)
	end

	local transparent = data:match('transparentcolor="(#%x%x%x%x%x%x)"')
	if transparent then
		tileset.transparentcolor = transparent
	end

	local margin = data:match('margin="(%d+)"')
	tileset.margin = margin and tonumber(margin) or 0

	local spacing = data:match('spacing="(%d+)"')
	tileset.spacing = spacing and tonumber(spacing) or 0

	local offset_x, offset_y = data:match('<tileoffset%s+[^>]-x="(-?%d+)"[^>]-y="(-?%d+)"')
	if offset_x and offset_y then
		tileset.tileoffset = {x = tonumber(offset_x), y = tonumber(offset_y)}
	else
	...
	for id, tile_xml in data:gmatch('<tile%s+id="(%d+)"(.-)</tile>') do
		...
		if tile.image or tile.properties then
			tiles[#tiles+1] = tile
		end
	end
...
function Map:init(path, plugins, ox, oy)
	...
	for i, tileset in ipairs(self.tilesets) do
		if tileset.filename then
			local tsx_path = utils.format_path(path .. tileset.filename)
			local tsx_data = love.filesystem.read(tsx_path)
			if not tsx_data then
				print("[STI] Failed to load TSX: " .. tsx_path .. " (from " .. path .. tileset.filename .. ")")
			end
			assert(tsx_data, "Could not load external Tileset: " .. tsx_path)
			local external = parseTSX(tsx_data)
			for k, v in pairs(external) do
				tileset[k] = v
			end
			tileset.filename = nil
		end

		tileset.tiles = tileset.tiles or {}
		tileset.terrains = tileset.terrains or {}
		...
		if tileset.image then
            ...
            if lg.isCreated then
                ...
                if not STI.cache[formatted_path] then
                    print("[STI] Loading image: " .. formatted_path)
...
function Map:setTiles(index, tileset, gid)
	...
	local imageW  = tileset.imagewidth or 0
	local imageH  = tileset.imageheight or 0
	...
	assert(tileW and tileW > 0, "Tileset missing tilewidth")
	assert(tileH and tileH > 0, "Tileset missing tileheight")

	if (imageW == 0 or imageH == 0) and type(tileset.image) == "userdata" and tileset.image.getDimensions then
		imageW, imageH = tileset.image:getDimensions()
	end
